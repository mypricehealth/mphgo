...
func TestResponsesUnmarshalJSON(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		err := json.Unmarshal([]byte(`{"results": [{"result": {"strVal":"1"}}, {"strVal":"2"}, {"strVal":"3"}], "status": 200}`), &r)
		require.NoError(t, err)

		assert.Equal(t, ErrorAndResultResponses[testStruct]{Results: []ErrorAndResult[testStruct]{{Result: testStruct{StrVal: "1"}}, {Result: testStruct{StrVal: "2"}}, {Result: testStruct{StrVal: "3"}}}, StatusCode: 200}, r)
	})
